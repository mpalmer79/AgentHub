...
def get_inventory_schema() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        create_tool_schema(
            ...
            properties={
                "product_id": string_prop(
                    "Specific product ID to order (optional, auto-detects if not specified)"
                ),
                "supplier_id": string_prop("Preferred supplier ID"),
                "auto_calculate": boolean_prop(
                    "Automatically calculate quantities based on forecast (default: true)"
                ),
            },
        ),
        create_tool_schema(
            ...
            properties={
                "optimization_type": string_prop(
                    "Type: all, overstock, understock, reorder_points, dead_stock"
                ),
        ...
        create_tool_schema(
            ...
            properties={
                "days_threshold": integer_prop(
                    "Days of stock threshold to consider slow-moving (default: 90)"
                ),
                "include_recommendations": boolean_prop(
                    "Include action recommendations (default: true)"
                ),
            },
        ),
    ]
