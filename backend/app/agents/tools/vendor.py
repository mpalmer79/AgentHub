...
class VendorTools:
    ...
    async def analyze_contracts(
        ...
        query = (
            supabase.table("vendor_contracts")
            .select("*")
            ...
            contracts = [
                {
                    ...
                    "contract_start": (datetime.utcnow() - timedelta(days=300)).strftime(
                        "%Y-%m-%d"
                    ),
                ...
                },
                {
                    ...
                    "monthly_cost": 800.00,
                    "annual_cost": 9600.00,
                    "contract_start": (datetime.utcnow() - timedelta(days=180)).strftime(
                        "%Y-%m-%d"
                    ),
                ...
                },
                {
                    ...
                    "monthly_cost": 450.00,
                    "annual_cost": 5400.00,
                    "contract_start": (datetime.utcnow() - timedelta(days=100)).strftime(
                        "%Y-%m-%d"
                    ),
                ...
                },
                {
                    ...
                    "contract_start": (datetime.utcnow() - timedelta(days=200)).strftime(
                        "%Y-%m-%d"
                    ),
        ...
        if expiring_within_days:
            ...
            contracts = [
                c for c in contracts if datetime.strptime(c["renewal_date"], "%Y-%m-%d") <= cutoff
            ]
    ...
    async def benchmark_pricing(
        self, vendor_name: str, category: str, current_price: float, service_description: str = ""
    ) -> Dict[str, Any]:
        ...
        market_data = {
            ...
            "supplies": {"default": {"low": 0.80, "median": 1.0, "high": 1.15}},
            "logistics": {"default": {"low": 0.85, "median": 1.0, "high": 1.20}},
        }
        ...
        multipliers = cat_data.get(
            subcategory, cat_data.get("default", {"low": 0.75, "median": 1.0, "high": 1.30})
        )
        ...
        if position == "above_market":
            leverage_points.append(
                f"Current pricing is {round((position_ratio - 1) * 100)}% above market median"
            )
        ...
        return {
            ...
            "category": category,
            "current_price": current_price,
            "market_analysis": {"low": market_low, "median": estimated_median, "high": market_high},
            "position": position,
    ...
    async def identify_savings(self, min_savings_threshold: float = 100.0) -> Dict[str, Any]:
        ...
        for contract in contracts:
            ...
            same_category = [
                c
                for c in contracts
                if c.get("category") == category and c.get("id") != contract.get("id")
            ]
            if same_category:
            ...
            opportunity["potential_annual_savings"] = round(
                opportunity["potential_monthly_savings"] * 12, 2
            )
            opportunity["potential_monthly_savings"] = round(
                opportunity["potential_monthly_savings"], 2
            )

            # Only include if meets threshold
            if opportunity["potential_annual_savings"] >= min_savings_threshold or opportunity[
                "priority"
            ] in ["high", "medium"]:
        ...
        opportunities.sort(
            key=lambda x: (priority_order.get(x["priority"], 3), -x["potential_annual_savings"])
        )
        ...
        return {
            ...
            "top_actions": [
                o["recommended_actions"][0] for o in opportunities[:3] if o["recommended_actions"]
            ],
            "analyzed_at": datetime.utcnow().isoformat(),
        }
    ...
    async def draft_negotiation(
        ...
        # Build the draft
        if tone == "firm":
            template["opening"] = template["opening"].replace(
...
[Company Name]""",
    ...
    async def track_renewals(self, days_ahead: int = 90) -> Dict[str, Any]:
        ...
        for contract in contracts:
            ...
            # Calculate key dates
            notice_deadline = datetime.strptime(contract["renewal_date"], "%Y-%m-%d") - timedelta(
                days=notice_days
            )
