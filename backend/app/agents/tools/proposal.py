...
class ProposalTools:
    ...
    async def generate_proposal(
        ...
        templates = {
            ...
            "brief": {"sections": ["summary", "scope", "pricing", "next_steps"], "tone": "concise"},
        }
...
3. Project begins within one week of acceptance""",
    ...
    def _generate_pricing_section(
        self, services: List[str], estimated_value: Optional[float]
    ) -> str:
...
Detailed pricing will be provided in the Cost Proposal section."""
    ...
    async def price_project(
        ...
        for item in scope_items:
            ...
            item_hours = (
                base["typical_hours"] / len(scope_items) if scope_items else base["typical_hours"]
            )
            ...
            pricing_options = [
                {
                    ...
                    "includes": scope_items[: len(scope_items) // 2 + 1]
                ...
                },
                {
                    ...
                    "includes": scope_items
                    + ["Priority support", "Extended warranty", "Quarterly reviews"],
    ...
    async def track_proposal(
        ...
        pipeline_value = sum(
            p.get("estimated_value", 0) for p in proposals if p.get("status") not in ["won", "lost"]
        )
        ...
        lost_value = sum(
            p.get("estimated_value", 0) for p in proposals if p.get("status") == "lost"
        )
        ...
        win_rate = (
            (sum(1 for p in completed if p.get("status") == "won") / len(completed) * 100)
            if completed
            else 0
        )
    ...
    async def analyze_win_rate(self, period_days: int = 180) -> Dict[str, Any]:
        ...
        if win_rate >= 60:
            ...
            insights.append(
                "Win rate needs improvement - consider proposal training or pricing review"
            )

        if avg_won_value < avg_lost_value:
            insights.append(
                "Higher-value deals more likely to be lost - consider value justification improvements"
            )

        if "Budget constraints" in loss_reasons:
            insights.append(
                "Budget is a common loss reason - consider offering tiered pricing options"
            )
