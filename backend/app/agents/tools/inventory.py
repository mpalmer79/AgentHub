...
class InventoryTools:
    ...
    async def forecast_demand(
        ...
        # Get products
        query = (
            supabase.table("inventory_products")
            .select("*")
        ...
        return {
            ...
            "summary": {
                ...
                "critical_stockout_risk": sum(
                    1 for f in forecasts if f["stockout_risk"] == "critical"
                ),
                "high_stockout_risk": sum(1 for f in forecasts if f["stockout_risk"] == "high"),
                "products_needing_reorder": sum(
                    1 for f in forecasts if f["stockout_risk"] in ["critical", "high"]
                ),
            },
    ...
    async def generate_purchase_order(
        ...
        expected_delivery = datetime.utcnow() + timedelta(
            days=selected_supplier.get("lead_time", 7)
        )
    ...
    async def optimize_inventory(
        self, optimization_type: str = "all", target_service_level: float = 0.95
    ) -> Dict[str, Any]:
        ...
        inventory_turns = round(
            ...
            / (
                total_current_value
                / (sum(p.get("forecasted_daily_demand", 0) * 15 for p in products) * 365)
            )
        ...
        return {
            ...
            "summary": {
                ...
                "products_overstocked": sum(
                    1
                    for o in optimizations
                    if any(r["type"] == "overstock" for r in o["recommendations"])
                ),
                "products_understocked": sum(
                    1
                    for o in optimizations
                    if any(r["type"] == "understock" for r in o["recommendations"])
                ),
    ...
    async def track_supplier(
        self, supplier_id: Optional[str] = None, include_performance: bool = True
    ) -> Dict[str, Any]:
        ...
        for supplier in suppliers:
            if include_performance and "performance" in supplier:
                ...
                supplier["tier"] = (
                    "preferred" if score >= 85 else "approved" if score >= 70 else "conditional"
                )
        ...
        for supplier in suppliers:
            if supplier.get("overall_score", 0) < 70:
                recommendations.append(
                    f"Review relationship with {supplier['name']} - below performance threshold"
                )
            if supplier.get("performance", {}).get("defect_rate", 0) > 2:
                recommendations.append(
                    f"Address quality issues with {supplier['name']} - {supplier['performance']['defect_rate']}% defect rate"
                )
        ...
        return {
            ...
            "summary": {
                ...
                "total_spend_90_days": sum(
                    s.get("performance", {}).get("total_spent_last_90_days", 0) for s in suppliers
                ),
                "average_on_time_rate": round(
                    sum(s.get("performance", {}).get("on_time_delivery_rate", 0) for s in suppliers)
                    / len(suppliers),
                    1,
                )
                if suppliers
    ...
    async def identify_slow_movers(
        self, days_threshold: int = 90, include_recommendations: bool = True
    ) -> Dict[str, Any]:
        ...
        for product in products:
            ...
            if days_of_stock >= days_threshold:
                ...
                if include_recommendations:
                    slow_mover["recommendations"] = self._get_slow_mover_recommendations(
                        category, product
                    )
        ...
        return {
            ...
            "summary": {
                ...
                "dead_stock_count": sum(
                    1 for s in slow_movers if s["slow_mover_category"] == "dead_stock"
                ),
                "very_slow_count": sum(
                    1 for s in slow_movers if s["slow_mover_category"] == "very_slow"
                ),
