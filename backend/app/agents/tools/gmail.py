...
class GmailTools:
    ...
    async def _make_request(
        ...
        if response.status_code in (200, 204):
            ...
            return {"error": f"Gmail API error: {response.status_code}", "details": response.text}
    ...
    async def get_emails(
        ...
        params = {"maxResults": min(max_results, 50), "q": " ".join(q_parts) if q_parts else None}
        ...
        return {"emails": emails, "count": len(emails), "has_more": len(messages) > max_results}
    ...
    async def triage_inbox(self, time_window_hours: int = 24) -> Dict[str, Any]:
    ...
    async def draft_response(
        self, email_id: str, response_body: str, include_original: bool = True
    ) -> Dict[str, Any]:
        ...
        draft_data = {"message": {"raw": raw, "threadId": email["thread_id"]}}
    ...
    async def schedule_followup(
        self, email_id: str, followup_date: str, followup_note: str
    ) -> Dict[str, Any]:
        ...
        supabase = get_supabase()
        followup = (
            supabase.table("email_followups")
            .insert(
                {
                    "user_id": self.user_id,
                    "email_id": email_id,
    ...
    async def extract_action_items(self, email_id: str) -> Dict[str, Any]:
    ...
    async def apply_label(self, email_id: str, label_name: str) -> Dict[str, Any]:
        ...
        result = await self._make_request(
            "POST", f"messages/{email_id}/modify", data={"addLabelIds": [label_id]}
        )
    ...
    async def mark_as_read(self, email_id: str) -> Dict[str, Any]:
        ...
        result = await self._make_request(
            "POST", f"messages/{email_id}/modify", data={"removeLabelIds": ["UNREAD"]}
        )
    ...
    async def archive_email(self, email_id: str) -> Dict[str, Any]:
        ...
        result = await self._make_request(
            "POST", f"messages/{email_id}/modify", data={"removeLabelIds": ["INBOX"]}
        )
    ...
    async def get_followups_due(self) -> Dict[str, Any]:
        ...
        return {"followups": result.data or [], "count": len(result.data or []), "as_of": today}
