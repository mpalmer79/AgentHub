...
def verify_quickbooks_signature(payload: bytes, signature: str, verifier_token: str) -> bool:
    ...
    expected = base64.b64encode(
        hmac.new(verifier_token.encode("utf-8"), payload, hashlib.sha256).digest()
    ).decode("utf-8")
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if user_id:
            supabase.table("users").update(
                {"stripe_customer_id": session.get("customer"), "subscription_status": "active"}
        ...
        if user.data:
            supabase.table("users").update({"subscription_status": subscription.get("status")}).eq(
                "id", user.data["id"]
            ).execute()
        ...
        if user.data:
            ...
            supabase.table("agent_subscriptions").update({"status": "cancelled"}).eq(
        ...
        if user.data:
            supabase.table("users").update({"subscription_status": "past_due"}).eq(
                "id", user.data["id"]
            ).execute()
...
async def quickbooks_webhook(
    request: Request, intuit_signature: Optional[str] = Header(None, alias="intuit-signature")
):
    """
    ...
    if not verify_quickbooks_signature(
        payload, intuit_signature, settings.QUICKBOOKS_WEBHOOK_VERIFIER_TOKEN
    ):
